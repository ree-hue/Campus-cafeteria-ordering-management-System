...
$query = "SELECT item_id, item_name, price, description FROM menu_items WHERE availability_status = 1 ORDER BY item_id LIMIT 6";
$result = pg_query($conn, $query);

if (!$result) {
    $error = "Error loading menu items: " . pg_last_error($conn);
    $menu_items = [];
} else {
    $menu_items = pg_fetch_all($result) ?? [];
}

// Get user's recent orders count
$orders_query = "SELECT COUNT(*) as order_count FROM orders WHERE user_id = $1";
$orders_result = pg_query_params($conn, $orders_query, array($user_id));
$order_count = 0;
if ($orders_result) {
    $order_row = pg_fetch_assoc($orders_result);
    $order_count = $order_row['order_count'] ?? 0;
}

// Get pending orders count
$pending_query = "SELECT COUNT(*) as pending_count FROM orders WHERE user_id = $1 AND status IN ('Pending', 'Paid', 'Preparing')";
