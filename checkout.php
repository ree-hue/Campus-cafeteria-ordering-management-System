...
if (isset($_POST['pay'])) {
    ...
    if ($payment_success) {
        ...
        $stmt = pg_prepare($conn, "insert_order", "INSERT INTO orders (user_id, order_date, total_amount, order_status) VALUES ($1, NOW(), $2, 'Paid') RETURNING order_id");
        $result = pg_execute($conn, "insert_order", array($user_id, $total));
        $row = pg_fetch_assoc($result);
        $order_id = $row['order_id'];

        
        $stmt_items = pg_prepare($conn, "insert_order_item", "INSERT INTO order_items (order_id, item_id, quantity, subtotal) VALUES ($1, $2, $3, $4)");
        ...
        foreach ($_SESSION['cart'] as $item) {
            ...
            pg_execute($conn, "insert_order_item", array($order_id, $item_id, $qty, $subtotal));
        }
